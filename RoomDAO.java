...
public class RoomDAO {
    ...
    // Insert new room
    public void addRoom(oop.calihat.mainmenu.Room room) {
    ...
    // Get all rooms
    public List<oop.calihat.mainmenu.Room> getAllRooms() {
        List<oop.calihat.mainmenu.Room> rooms = new ArrayList<>();
        try (MongoCursor<Document> cursor = roomCollection.find().iterator()) {
            while (cursor.hasNext()) {
                Document doc = cursor.next();
                rooms.add(new oop.calihat.mainmenu.Room(
    ...
    // Find room by ID
    public oop.calihat.mainmenu.Room getRoomById(String roomId) {
        Document doc = roomCollection.find(eq("roomId", roomId)).first();
        if (doc != null) {
            return new oop.calihat.mainmenu.Room(
                    doc.getString("roomId"),
                    doc.getString("roomNo"),
                    doc.getString("type"),
                    doc.getString("status"),
